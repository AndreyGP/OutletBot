...
public class JsonUtil {
    private WebHookOutletBot bot;
    private String botToken = bot.getBotToken();
    private Message inMessage;

    @Lazy
    public JsonUtil(WebHookOutletBot bot) {
        this.bot = bot;
    }
    ...
    private List<String> getJSONStringsList() throws MalformedURLException {
        ...
        try (BufferedReader in = new BufferedReader(new InputStreamReader(url.openStream()))) {
            ...
            try {
                throw new JsonUploadException(bot, inMessage.getChatId().toString(), "Ошибка загрузки файла со стороны пользоователя");
        ...
        try (BufferedReader outToString = new BufferedReader(new InputStreamReader(downoload.openStream()))) {
            ...
            try {
                throw new JsonUploadException(bot, inMessage.getChatId().toString(), "Ошибка загрузки файла со стороны сервера");
            } catch (JsonUploadException jsonUploadException) {
                jsonUploadException.printStackTrace();
            }
        }
